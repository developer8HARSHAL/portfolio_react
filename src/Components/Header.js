...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 lg:left-10 lg:right-10 z-50 transition-all duration-500 ${
        isScrolled
...
>
  ...
  <div
    className={`absolute top-0 right-0 h-full w-full sm:w-80 bg-slate-900 backdrop-blur-2xl border-l border-white/30 shadow-2xl transition-transform duration-500 ${
      ...
      <nav className="flex-1 space-y-2">
        {navLinks.map((item, index) => (
          <button
            key={item.id}
            onClick={() => handleScrollTo(item.to)}
            className={`w-full text-left px-6 py-4 text-[17px] font-semibold rounded-xl transition-all duration-300 cursor-pointer ${
              activeSection === item.id
